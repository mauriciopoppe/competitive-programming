...
int sweeping_line(vector<segment> &seg)
{
     ...
     vector<point> ip;
     set<segment> sl;
     set<segment>::iterator it, above, below, now;
     point p;
     segment s;
     ...
     while(!q.empty())
     {
         ...
         if(e.type == LEFT)
         {
             ...
             {
                 above = ++now;
                 if(above != sl.end() && segment_intersection(above->p1, above->p2, e.seg.p1, e.seg.p2))
                 {
                     p = intersect_segments(above->p1, above->p2, e.seg.p1, e.seg.p2);                          
                     ip.push_back(p);
                 }
                 --now;
             }
             ...
             {
                 below = --now;
                 if(below != sl.begin() && segment_intersection(below->p1, below->p2, e.seg.p1, e.seg.p2))
                 {
                     p = intersect_segments(below->p1, below->p2, e.seg.p1, e.seg.p2);                          
                     ip.push_back(p);
                 }
                 ++now;
             }
         ...
         {
             now = sl.find(e.seg);
             if(now == sl.begin() || now == sl.end())
                 continue;
             
             above = ++now;
             --now;
             below = --now;
             if(above != sl.end() && below != sl.begin())
                  if(segment_intersection(above->p1, above->p2, below->p1, below->p2))
                  {
                     p = intersect_segments(above->p1, above->p2, below->p1, below->p2);                          
                     ip.push_back(p);                                                     
                  }
             sl.erase(e.seg);
         }
